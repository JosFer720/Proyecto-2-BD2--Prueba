...
PRIMARY_LABEL = "Tag"
ALLOWED_LABELS = {"Tag", "Official", "Trending", "Deprecated"}
ALLOWED_FILTER_KEYS = {"id", "name", "usageCount", "isOfficial", "createdAt"}
AGGREGATE_FIELDS = {"usageCount"}
...
def create_tag(body: TagCreate, session: Session = Depends(get_session)):
    uid = str(uuid.uuid4())
    query = (
        "CREATE (n:Tag {id: $id, name: $name, createdAt: date($createdAt), "
        "usageCount: $usageCount, isOfficial: $isOfficial, relatedTags: $relatedTags}) "
        "RETURN n"
    )
...
def create_tag_multi_label(body: MultiLabelCreate, session: Session = Depends(get_session)):
    if not body.labels:
...
def search_tags(
    ...
    isOfficial: Optional[bool] = Query(None),
    usageCount: Optional[int] = Query(None),
    ...
    if isOfficial is not None:
        filters["isOfficial"] = isOfficial
    if usageCount is not None:
        filters["usageCount"] = usageCount

    where, params = _build_where(filters)
    query = f"MATCH (n:Tag) {where} RETURN n ORDER BY n.name SKIP $skip LIMIT $limit"
    params["skip"] = skip
    params["limit"] = limit
    result = session.run(query, params)
...
def aggregate_tags(
    field: Optional[str] = Query(None, description="Field to aggregate: usageCount"),
    session: Session = Depends(get_session),
):
    ...
    if field:
        query = (
            f"MATCH (n:Tag) RETURN count(n) AS count, "
            f"avg(n.{field}) AS avg_{field}, sum(n.{field}) AS sum_{field}"
        )
    else:
        query = "MATCH (n:Tag) RETURN count(n) AS count"
...
@router.get("/{node_id}")
def get_tag(node_id: str, session: Session = Depends(get_session)):
    result = session.run("MATCH (n:Tag {id: $id}) RETURN n", {"id": node_id})
    record = result.single()
    if not record:
...
def update_tag_properties(node_id: str, body: PropertiesUpdate, session: Session = Depends(get_session)):
    set_parts = []
    params = {"id": node_id}
    ...
    query = f"MATCH (n:Tag {{id: $id}}) SET {set_clause} RETURN n"
    result = session.run(query, params)
    record = result.single()
    if not record:
        raise HTTPException(status_code=404, detail="Tag not found")
...
def batch_update_tag_properties(body: BatchPropertyUpdate, session: Session = Depends(get_session)):
    ...
    set_clause = ", ".join(set_parts)
    query = f"MATCH (n:Tag) {where} SET {set_clause} RETURN n"
    result = session.run(query, params)
    return [_node_to_dict(r) for r in result]
...
def delete_tag_properties(node_id: str, body: PropertyNamesDelete, session: Session = Depends(get_session)):
    remove_parts = [f"n.{name}" for name in body.property_names]
    remove_clause = ", ".join(remove_parts)
    query = f"MATCH (n:Tag {{id: $id}}) REMOVE {remove_clause} RETURN n"
...
def batch_delete_tag_properties(body: BatchPropertyDelete, session: Session = Depends(get_session)):
    where, params = _build_where(body.filter)
    remove_parts = [f"n.{name}" for name in body.property_names]
    remove_clause = ", ".join(remove_parts)
    query = f"MATCH (n:Tag) {where} REMOVE {remove_clause} RETURN n"
    result = session.run(query, params)
    return [_node_to_dict(r) for r in result]
...
def delete_tag(node_id: str, session: Session = Depends(get_session)):
    result = session.run("MATCH (n:Tag {id: $id}) DETACH DELETE n RETURN count(n) AS deleted", {"id": node_id})
    record = result.single()
    if record["deleted"] == 0:
        raise HTTPException(status_code=404, detail="Tag not found")
    return {"deleted": True}
...
@router.delete("/batch")
def batch_delete_tags(body: BatchDelete, session: Session = Depends(get_session)):
    where, params = _build_where(body.filter)
    query = f"MATCH (n:Tag) {where} DETACH DELETE n RETURN count(n) AS deleted"
    result = session.run(query, params)
    record = result.single()
